...
def eval_multisession(model, database_sets, query_sets):
    ...
    # add by cjf visualization of embeddings via t-SNE CCL code
        ...
        for j in range(len(indices[0])):
            if indices[0][j] in true_neighbors:
                if j == 0:
                    ...
                    # cos_similarity = np.dot(queries_output[i], database_output[indices[0][j]]) / (np.linalg.norm(queries_output[i]) * np.linalg.norm(database_output[indices[0][j]]))
