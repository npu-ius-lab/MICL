...
class OxfordDataset(Dataset):
    ...
    def get_negatives(self, ndx):
        return self.queries[ndx].negatives
    ...
    def add_memory(self, memory):
        if memory != None:
            memory_queries_dict = memory.get_tuples(self.__len__())   ##把memory放到new_dataset后面
            self.queries.update(memory_queries_dict)##更新字典
...
class TrainingTuple:
    ...
    def __init__(self, id: int, timestamp: int, rel_scan_filepath: str, positives: np.ndarray,
                 non_negatives: np.ndarray,negatives: np.ndarray,position: np.ndarray):
