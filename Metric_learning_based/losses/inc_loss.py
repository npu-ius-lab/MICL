...
class New_MI:
    def __init__(self):
        self.orig_weight = configs.train.loss.incremental.weight #Nothing
        self.weight = configs.train.loss.incremental.weight #Nothing
        self.margin = configs.train.loss.incremental.margin #Nothing
    ...
    def __call__(self, current_new_rep,pos_mask ,neg_mask):
        current_new_rep = nn.functional.normalize(current_new_rep, dim=1)
        ...
        for i in range(batch_size):
            ...
            # if positive_i.dim() != 0 or len(negative_i) != (batch_size - 2):
            #     print(positive_i)
...
class Seen_MI:
    ...
    def __call__(self, mem_rep ,memories_pos_mask ,memories_neg_mask):
        mem_rep = nn.functional.normalize(mem_rep, dim=1)
...
class Current_Past_MI:
    ...
    def __call__(self, old_mem_rep, new_mem_rep ,memories_pos_mask ,memories_neg_mask):
        old_mem_rep = nn.functional.normalize(old_mem_rep, dim=1)
        new_mem_rep = nn.functional.normalize(new_mem_rep, dim=1)
